...
class curseMySqlDB:
    ...
    def queryMysqlvJAM(self, query): #JAM
        if self.connection:
            c= self.connection.cursor()
            try:
                c.execute(query)
...
def testLoginQuery(log_info_map, q_str,connection = None):
    login_obj = curseMySqlDB(
        log_info_map["dbname"],   log_info_map["username"], 
        log_info_map["password"], log_info_map["host"])
    status = login_obj.connectToDB()
    if 'fail' in status:        return "FAIL CONNECT"
    q_results = login_obj.queryMysqlvJAM(q_str)
    if 'error' in q_results:    return "FAIL QUERY"
    else: return q_results
...
#function to call when what to query the database
def queryMysqlvJAM(loginInfo, q_string):
    con = None
